...
9. `08-auth`: This one is for mastery of betterauth, managing users, etc, all in a CRUD interface (using remote functions)
...
- This is now a `Bun MONOREPO`, meaning that each workspace is a recursive package and can access each other's exports. The `packages` directory contains shared code that i'll start using like signup schemas and forms.
